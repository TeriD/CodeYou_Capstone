...
# Add GeoJSON layers to panes
county_layer = folium.GeoJson(
    counties_geojson, name='Counties', pane='county_pane').add_to(m)
roads_layer = folium.GeoJson(
    roads_geojson, name='Roads', pane='road_pane').add_to(m)
...
def zoom_to_county(county_name):
...
dropdown.param.watch(lambda event: zoom_to_county(event.new), 'value')
